...
class Serpiente:
    ...
    def crear_serpiente(self):
        for posicion in POSICIONES_INICIALES:
            self.anyadir_segmento(posicion)
    
    def extender(self):
        self.anyadir_segmento(self.segmentos[-1].position())
        
    def anyadir_segmento(self, posicion_segmento):
        nuevo_segmento = turtle.Turtle("square") #Acepta el shape en el constructor de clase.
        nuevo_segmento.penup()
        nuevo_segmento.color("white")
        nuevo_segmento.goto(posicion_segmento)
